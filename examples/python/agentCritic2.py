...
class AgentCritic:
    def __init__(self, num_actions, epsilon=1, epsilon_min=0.1, epsilon_decay=0.9995):
        ...
        self.model_savefolder_actor = f"F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/actor{self.episode}"
        self.model_savefolder_critic = f"F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/critic{self.episode}"

        self.model_savefolder_actor_backup = f"F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/actor{self.episode-2}"
        self.model_savefolder_critic_backup = f"F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/critic{self.episode-2}"

        if load:
            print("Loading model actor from: ", self.model_savefolder_actor, 
                  "\n Loading model critic from: ",self.model_savefolder_critic)
            self.actor = tf.keras.models.load_model("F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/actor2",  compile=False)
            self.critic = tf.keras.models.load_model("F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/critic2",  compile=False)
    ...
    def choose_action(self, state):
        ...
        try:
            ...
            self.actor = tf.keras.models.load_model(f"F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/actor{self.episode-2}",  compile=False)
            self.critic = tf.keras.models.load_model(f"F:/SIiUM3/ViZDoom/model_actor2_a38/model_backup/critic{self.episode-2}",  compile=False)
            self.actor.compile(loss = 'mse', optimizer = Adam(learning_rate_actor),  metrics=['accuracy'])
            self.critic.compile(loss = 'mse', optimizer = Adam(learning_rate_critic), metrics=['accuracy'])
